...
class TicTacToeState:
    ...
    board_state = beaker.GlobalStateValue(
...
def has_player_won(state: pt.Expr) -> pt.Expr:
    return pt.If(
        pt.Or(
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[0])) == pt.Int(
                WINING_STATES[0]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[1])) == pt.Int(
                WINING_STATES[1]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[2])) == pt.Int(
                WINING_STATES[2]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[3])) == pt.Int(
                WINING_STATES[3]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[4])) == pt.Int(
                WINING_STATES[4]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[5])) == pt.Int(
                WINING_STATES[5]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[6])) == pt.Int(
                WINING_STATES[6]),
            pt.BitwiseAnd(state, pt.Int(WINING_STATES[7])) == pt.Int(WINING_STATES[7])), pt.Int(1), pt.Int(0))


@pt.Subroutine(pt.TealType.uint64)
def is_tie() -> pt.Expr:
    state_x = app.state.player_x_state
    state_o = app.state.player_o_state
    return pt.Int(511) == pt.BitwiseOr(state_x, state_o)
...
def select_position_for_o(available_positions: pt.Expr) -> pt.Expr:
    ...
    return pt.Seq(
        [pt.For(brojac.store(pt.Int(0)), brojac.load() < pt.Int(9), brojac.store(brojac.load()+pt.Int(1))).
            Do(
            pt.Seq([
                pt.If(pt.GetBit(available_positions, brojac.load()) == pt.Int(1)).
                Then(
                    pt.Seq(
...
@app.external
def play_action_logic(position_index: pt.abi.Uint64, *, output: pt.abi.String) -> pt.Expr:
    ...
    player_o_move = pt.Seq([
        app.state.player_o_state.set(pt.BitwiseOr(app.state.player_o_state.get(), pt.ShiftLeft(
            pt.Int(1), select_position_for_o(available_positions(app.state.player_x_state.get(), app.state.player_o_state.get()))))),
        pt.If(has_player_won(app.state.player_o_state),
              app.state.game_status.set(pt.Int(2)))
    ])

    player_x_move = pt.Seq([
        app.state.player_x_state.set(pt.BitwiseOr(state_x, game_action)),
        pt.If(has_player_won(app.state.player_x_state),
    ...
    return pt.Seq([
        ...
        pt.Cond(
            [app.state.game_status == pt.Int(
                0), output.set(app.state.board_state)],
            [app.state.game_status == pt.Int(1), pt.Seq([output.set(
                "You have succcesfully won the game!")])],
            [app.state.game_status == pt.Int(
                2), output.set("You have lost the game!")],
...
def getBoardState() -> pt.Expr:
    ...
    return pt.Seq(
        [app.state.board_state.set_default(),
         pt.For(brojac.store(pt.Int(0)), brojac.load() < pt.Int(9), brojac.store(brojac.load()+pt.Int(1))).
            Do(
            pt.Seq([
                pt.If(pt.GetBit(state_x, brojac.load()) == pt.Int(1)).
                Then(
                    pt.Seq(
                        app.state.board_state.set(
                            pt.Concat(app.state.board_state.get(), pt.Bytes("X")))
                    )
                ).
                ElseIf(pt.GetBit(state_o, brojac.load()) == pt.Int(1)).
                Then(pt.Seq(
                    app.state.board_state.set(
                        pt.Concat(app.state.board_state.get(), pt.Bytes("O")))
                )).
                Else(app.state.board_state.set(
                    pt.Concat(app.state.board_state.get(), pt.Bytes("-")))),

                pt.If(pt.Or(brojac.load() == pt.Int(2), brojac.load()
                      == pt.Int(5), brojac.load() == pt.Int(8))).
                Then(app.state.board_state.set(
                    pt.Concat(app.state.board_state.get(), pt.Bytes("\n"))))
            ]
            )
        ), pt.Return()])
